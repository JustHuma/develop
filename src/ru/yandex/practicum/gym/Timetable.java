...
public class Timetable {
    ...
    public List<TrainingSession> getTrainingSessionsForDay(DayOfWeek dayOfWeek) {
        ...
        if (!timetable.containsKey(dayOfWeek)) return result;
    ...
    public List<TrainingSession> getTrainingSessionsForDayAndTime(DayOfWeek dayOfWeek, TimeOfDay timeOfDay) {
        if (!timetable.containsKey(dayOfWeek)) return new ArrayList<>();
        TreeMap<TimeOfDay, List<TrainingSession>> dayMap = timetable.get(dayOfWeek);
        if (!dayMap.containsKey(timeOfDay)) return new ArrayList<>();
